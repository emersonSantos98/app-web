...
function resolverUserStatusName(stat: string) {
...
function addNewUser(user: IUser) {
  userService.store.updateOrAddUser(user)
  fetchUsers()
}

function editUser(user: IUser) {
  userService.store.setUser(user)
  ...
  <section>
    <VCard>
      ...
      <VDataTableServer
        ...
        :items="userService.store.users"
        :items-length="userService.store.totalUsers"
        :headers="headers"
        class="text-no-wrap"
      ...
      >
        ...
        <template #item.user="{ item }">
          <div class="d-flex align-center">
            <VAvatar
              ...
              :color="!item.raw.avatar ? resolveUserRoleVariant(item.raw.role.name).color : undefined"
              class="me-3"
            >
              <VImg
            ...
            <div class="d-flex flex-column">
              <h6 class="text-body-1 font-weight-medium">
                <RouterLink
                  :to="{ name: 'login', params: { id: item.raw.id } }"
        ...
        <template #item.status="{ item }">
          ...
          >
            {{ resolverUserStatusName(item.raw.status) }}
          </VChip>
        </template>
        ...
        <template #item.actions="{ item }">
          ...
          >
            ...
            <VMenu activator="parent">
              <VList>
                <VListItem :to="{ name: 'login', params: { id: item.raw.id } }">
